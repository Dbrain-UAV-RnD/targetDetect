...
# ROI 설정 - 중요!
MIN_ROI_SIZE = 50  # 최소 ROI 크기
DEFAULT_ROI_SIZE = 120  # 기본 ROI 크기
MAX_ROI_SIZE = 200  # 최대 ROI 크기
...
def create_safe_tracker(frame, init_bbox):
    """안전한 트래커 생성 - 다양한 트래커 시도"""
    x, y, w, h = init_bbox
    ...
    if w < MIN_ROI_SIZE or h < MIN_ROI_SIZE:
        ...
        cx = x + w / 2
        cy = y + h / 2
        new_size = max(MIN_ROI_SIZE, min(w, h))
        x = int(cx - new_size / 2)
        y = int(cy - new_size / 2)
        w = h = new_size
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                return None, init_bbox, ""
    
    # 트래커 초기화
    try:
        success = tracker.init(frame, init_bbox)
        if not success:
            print(f"Failed to initialize {tracker_name} tracker")
            return None, init_bbox, ""
    except Exception as e:
        print(f"Error initializing {tracker_name}: {e}")
        return None, init_bbox, ""
        
    return tracker, init_bbox, tracker_name

def process_new_coordinate(frame):
    global latest_point, new_point_received, tracker, tracking, roi, current_frame, zoom_level
    global center_x, center_y, prev_center_x, prev_center_y, velocity_x, velocity_y, tracker_fail_count
    ...
    if new_point_received:
        ...
        if 0 <= x < frame.shape[1] and 0 <= y < frame.shape[0]:
            ...
            # 줌 레벨에 따라 조정
            roi_size = int(roi_size / zoom_level)
            ...
            # 너무 작으면 다시 조정
            if actual_width < MIN_ROI_SIZE or actual_height < MIN_ROI_SIZE:
                # 다른 방향으로 확장
                if left == 0:
                    right = min(frame.shape[1], left + MIN_ROI_SIZE)
                if top == 0:
                    bottom = min(frame.shape[0], top + MIN_ROI_SIZE)
                if right == frame.shape[1]:
                    left = max(0, right - MIN_ROI_SIZE)
                if bottom == frame.shape[0]:
                    top = max(0, bottom - MIN_ROI_SIZE)
                    
                actual_width = right - left
                actual_height = bottom - top
            
            roi = (left, top, actual_width, actual_height)
            
            center_x = left + actual_width / 2
            center_y = top + actual_height / 2
            ...
            # 안전한 트래커 생성
            tracker, roi, tracker_name = create_safe_tracker(frame, roi)
            if tracker is not None:
                tracking = True
                print(f"Using {tracker_name} tracker with ROI: {roi}")
            else:
                tracking = False
...
def main():
    ...
    global zoom_level, zoom_command, zoom_center, center_x, center_y
    ...
    try:
        while True:
                    ...
                    if frame_counter % 3 == 0:
                        ...
                        if not success:
                            ...
                            if tracker_fail_count > 5:
                                tracking = False
                                print("Tracking lost - too many failures")
                    else:
            ...
            if roi is not None and tracking:
                roi_info = f"ROI: {roi[2]}x{roi[3]}"
                cv2.putText(display_frame, roi_info, (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 0), 2)
